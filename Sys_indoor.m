...
%% State: x y h
    ...
    x_pre(1,1) = x(1)*opt.std_loc(1) + opt.mean_loc(1);
    x_pre(2,1) = x(2)*opt.std_loc(2) + opt.mean_loc(2);
    %x_pre = [x(1);x(2)];
    ...
    fTmp = calib*x_f_temp;
    x_(1) = ((fTmp(1)/fTmp(3)) - opt.mean_loc(1))/opt.std_loc(1);
    x_(2) = ((fTmp(2)/fTmp(3)) - opt.mean_loc(2))/opt.std_loc(2);
    x_(3) = x_w_(3);
end